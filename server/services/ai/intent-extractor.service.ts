...
/**
 * Phase 7 UX Correction: Determines whether customer query explicitly asks for a price reduction or offer.
 * Explicitly separates:
 * A) internal price hesitation ("Why is this expensive?", "Is this worth the price?") -> false
 * B) explicit customer offer request ("Is there any price reduction on this?", "Can you make it cheaper?") -> true
 */
export function isExplicitOfferRequest(query: string): boolean {
  const lower = query.toLowerCase().trim();

  // Negative guards - must NOT trigger OFFER_REQUEST:
  // "why is this expensive", "is this worth the price", "difference between", comparison, feature inquiries
  if (/^(why\s+is\s+(this|it|option|that)\s+(so\s+)?(expensive|costly|pricier))/i.test(lower)) {
    return false;
  }
  if (/\b(worth\s+(the\s+)?(money|price|it)|is\s+(it|this|that)\s+worth)\b/i.test(lower)) {
    return false;
  }
  if (/\b(compare|versus|\bvs\b|difference\s+between)\b/i.test(lower)) {
    return false;
  }
  if (/^(does\s+(it|this|that)|has\s+(it|this|that)|what\s+is\s+the\s+(battery|anc|spec|features?|warranty))/i.test(lower)) {
    return false;
  }

  // Explicit patterns asking for price reduction / discount / offer
  const explicitPatterns = [
    /\b(price\s+reduction|reduce\s+(the\s+)?price|reduction\s+on\s+this|reduced\s+price)\b/i,
    /\b(is\s+there\s+(a\s+|an\s+|any\s+)?(discount|offer|deal|price\s+reduction|reduced\s+price))\b/i,
    /\b(can\s+(you|i)\s+(get|make)\s+(this|it)?\s*cheaper|make\s+it\s+cheaper|get\s+it\s+cheaper)\b/i,
    /\b(can\s+i\s+get\s+(this|it)\s+cheaper|get\s+this\s+cheaper)\b/i,
    /\b(any\s+(discount|offer|deal|price\s+cut)\s*(available|on\s+this)?)\b/i,
    /\b(is\s+there\s+(a\s+|an\s+|any\s+)?(discount|offer|deal|price\s+reduction))\b/i,
    /\b(can\s+you\s+(give|offer|do)\s+(me\s+)?(a\s+)?(discount|deal|better\s+price|offer|price\s+reduction))\b/i,
    /\b(can\s+i\s+get\s+(a\s+)?(discount|better\s+price|deal|special\s+offer|lower\s+price|price\s+reduction))\b/i,
    /\b(any\s+offer\s+available|is\s+there\s+an\s+offer)\b/i,
    /\b(give\s+me\s+(a\s+)?(discount|better\s+price|deal|offer))\b/i,
    /\b(can\s+you\s+lower\s+the\s+price|lower\s+the\s+price)\b/i,
    /\b(best\s+price\s+(available|for\s+this|you\s+can\s+give|you\s+can\s+do))\b/i,
  ];
...
export class IntentExtractorService {
  ...
  async extractIntent(
    ...
    // Fast path: Explicit Offer / Price Reduction Request (Phase 7 UX Correction)
    // Runs deterministically with ZERO Gemini calls
  ...
  public detectIntentMode(
    ...
    // Phase 7 UX: Explicit Commercial Offer / Price Reduction Request
    // Triggered ONLY when customer explicitly requests a price reduction, discount, or offer
    if (isExplicitOfferRequest(query)) {
